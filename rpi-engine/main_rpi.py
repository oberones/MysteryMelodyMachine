"""
Alternative main.py with enhanced Raspberry Pi MIDI support.

This version uses the robust MIDI classes that handle ALSA connection issues
more gracefully on Raspberry Pi systems.
"""
import time
import logging
import argparse
...
# Import the robust MIDI classes
from rpi_midi_utils import RobustMidiInput, RobustMidiOutput, get_system_info
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Mystery Music Engine Phase 7 (Raspberry Pi Compatible)")
    p.add_argument("--config", default="config.yaml", help="Path to config YAML")
    p.add_argument("--log-level", default=None, help="Override log level (DEBUG/INFO/...)")
    p.add_argument("--use-robust-midi", action="store_true", 
                   help="Use robust MIDI classes for Raspberry Pi compatibility")
...
def main(argv: Optional[list[str]] = None):
    ...
    log.info(f"system_info {system_info}")
    ...
    log.info(f"robust_midi_enabled={args.use_robust_midi}")
    ...
    # Initialize MIDI output (with robust handling if requested)
    if args.use_robust_midi:
        log.info("Using robust MIDI classes for Raspberry Pi compatibility")
        midi_output = RobustMidiOutput.create(
            cfg.midi.output_port, 
            cfg.midi.output_channel,
            retry_attempts=5  # More retries for Pi
        )
    else:
        # Use original MIDI classes
    ...
    # Initialize MIDI input (with robust handling if requested)
    try:
        if args.use_robust_midi:
            midi = RobustMidiInput.create(
                cfg.midi.input_port, 
                router.route,
                retry_attempts=5  # More retries for Pi
            )
