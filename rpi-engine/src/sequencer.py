...
class Sequencer:
    ...
    def _generate_step_note(self, step: int):
        ...
        # Simple pattern: active on steps 0, 2, 4, 6
        is_active_step = step % 2 == 0
        
        if is_active_step and random.random() < note_prob:
            ...
            try:
                ...
                log.debug(f"note_generated step={step} note={note} velocity={velocity} prob={note_prob:.2f}")
            except Exception as e:
