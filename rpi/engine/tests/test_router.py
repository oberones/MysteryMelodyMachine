...
def collect_events(cfg_path="rpi/engine/config.yaml"):
...
def test_note_mapping_triggers_event():
    ...
    msg = mido.Message("note_on", note=60, velocity=100, channel=cfg.midi.channel - 1)
    router.route(msg)
    assert len(events) == 1
    evt = events[0]
...
def test_cc_mapping_triggers_event():
    router, events, cfg = collect_events()
    msg = mido.Message("control_change", control=24, value=64, channel=cfg.midi.channel - 1)
    router.route(msg)
    assert len(events) == 1
    evt = events[0]
...
def test_channel_filter():
    router, events, cfg = collect_events()
    other_channel = (cfg.midi.channel % 16)  # different 0-based channel
...
def test_note_off_ignored_phase1():
    router, events, cfg = collect_events()
    msg_on = mido.Message("note_on", note=60, velocity=100, channel=cfg.midi.channel - 1)
    router.route(msg_on)
    msg_off = mido.Message("note_off", note=60, velocity=0, channel=cfg.midi.channel - 1)
